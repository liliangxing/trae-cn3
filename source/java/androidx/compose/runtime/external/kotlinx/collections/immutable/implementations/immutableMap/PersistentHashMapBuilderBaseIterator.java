...
public class PersistentHashMapBuilderBaseIterator<K, V, T> extends PersistentHashMapBaseIterator<K, V, T> implements Iterator<T>, KMutableIterator {
    ...
    private final void resetPath(int keyHash, TrieNode<?, ?> node, K key, int pathIndex) {
        ...
        if (node.hasEntryAt$runtime(indexSegment)) {
            getPath()[pathIndex].reset(node.getBuffer(), node.entryCount$runtime() * 2, node.entryKeyIndex$runtime(indexSegment));
            setPathLastIndex(pathIndex);
        } else {
            int nodeIndex$runtime = node.nodeIndex$runtime(indexSegment);
            TrieNode<?, ?> nodeAtIndex$runtime = node.nodeAtIndex$runtime(nodeIndex$runtime);
            getPath()[pathIndex].reset(node.getBuffer(), node.entryCount$runtime() * 2, nodeIndex$runtime);
            resetPath(keyHash, nodeAtIndex$runtime, key, pathIndex + 1);
        }
    }
