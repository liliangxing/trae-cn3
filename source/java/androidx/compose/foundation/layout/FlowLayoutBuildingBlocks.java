...
public final class FlowLayoutBuildingBlocks {
    ...
    public final WrapEllipsisInfo getWrapEllipsisInfo(WrapInfo wrapInfo, boolean hasNext, int lastContentLineIndex, int totalCrossAxisSize, int leftOverMainAxis, int nextIndexInLine) {
        ...
        if (!wrapInfo.getIsLastItemInContainer() || (ellipsisInfo$foundation_layout = this.overflow.ellipsisInfo$foundation_layout(hasNext, lastContentLineIndex, totalCrossAxisSize)) == null) {
            return null;
        }
        ellipsisInfo$foundation_layout.setPlaceEllipsisOnLastContentLine(lastContentLineIndex >= 0 && (nextIndexInLine == 0 || (leftOverMainAxis - IntIntPair.m127getFirstimpl(ellipsisInfo$foundation_layout.getEllipsisSize()) >= 0 && nextIndexInLine < this.maxItemsInMainAxis)));
    ...
    /*
        Code decompiled incorrectly, please refer to instructions dump.
    */
    public final WrapInfo m964getWrapInfoOpUlnko(boolean nextItemHasNext, int nextIndexInLine, long leftOver, IntIntPair nextSize, int lineIndex, int totalCrossAxisSize, int currentLineCrossAxisSize, boolean isWrappingRound, boolean isEllipsisWrap) {
        boolean z;
        int i = totalCrossAxisSize + currentLineCrossAxisSize;
        if (nextSize == null) {
            return new WrapInfo(true, true);
        }
        if (this.overflow.getType$foundation_layout() != FlowLayoutOverflow.OverflowType.Visible && (lineIndex >= this.maxLines || IntIntPair.m128getSecondimpl(leftOver) - IntIntPair.m128getSecondimpl(nextSize.getPackedValue()) < 0)) {
            return new WrapInfo(true, true);
        }
        if (nextIndexInLine != 0 && (nextIndexInLine >= this.maxItemsInMainAxis || IntIntPair.m127getFirstimpl(leftOver) - IntIntPair.m127getFirstimpl(nextSize.getPackedValue()) < 0)) {
            if (isWrappingRound) {
                return new WrapInfo(true, true);
            }
            return new WrapInfo(true, m964getWrapInfoOpUlnko(nextItemHasNext, 0, IntIntPair.m123constructorimpl(Constraints.getMaxWidth-impl(this.constraints), (IntIntPair.m128getSecondimpl(leftOver) - this.crossAxisSpacing) - currentLineCrossAxisSize), IntIntPair.m120boximpl(IntIntPair.m123constructorimpl(IntIntPair.m127getFirstimpl(nextSize.getPackedValue()) - this.mainAxisSpacing, IntIntPair.m128getSecondimpl(nextSize.getPackedValue()))), lineIndex + 1, i, 0, true, false).getIsLastItemInContainer());
        }
        int max = totalCrossAxisSize + Math.max(currentLineCrossAxisSize, IntIntPair.m128getSecondimpl(nextSize.getPackedValue()));
        IntIntPair m973ellipsisSizeF35zmw$foundation_layout = isEllipsisWrap ? null : this.overflow.m973ellipsisSizeF35zmw$foundation_layout(nextItemHasNext, lineIndex, max);
        if (m973ellipsisSizeF35zmw$foundation_layout != null) {
            m973ellipsisSizeF35zmw$foundation_layout.getPackedValue();
            if (nextIndexInLine + 1 >= this.maxItemsInMainAxis || ((IntIntPair.m127getFirstimpl(leftOver) - IntIntPair.m127getFirstimpl(nextSize.getPackedValue())) - this.mainAxisSpacing) - IntIntPair.m127getFirstimpl(m973ellipsisSizeF35zmw$foundation_layout.getPackedValue()) < 0) {
                z = true;
                if (z) {
                ...
                WrapInfo m964getWrapInfoOpUlnko = m964getWrapInfoOpUlnko(false, 0, IntIntPair.m123constructorimpl(Constraints.getMaxWidth-impl(this.constraints), (IntIntPair.m128getSecondimpl(leftOver) - this.crossAxisSpacing) - Math.max(currentLineCrossAxisSize, IntIntPair.m128getSecondimpl(nextSize.getPackedValue()))), m973ellipsisSizeF35zmw$foundation_layout, lineIndex + 1, max, 0, true, true);
                return new WrapInfo(m964getWrapInfoOpUlnko.getIsLastItemInContainer(), m964getWrapInfoOpUlnko.getIsLastItemInContainer());
            }
        }
        z = false;
        if (z) {
