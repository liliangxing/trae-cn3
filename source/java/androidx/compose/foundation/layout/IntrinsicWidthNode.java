...
final class IntrinsicWidthNode extends IntrinsicSizeModifier {
    ...
    public long mo1002calculateContentConstraintsl58MMJ0(MeasureScope measureScope, Measurable measurable, long j) {
        ...
        if (this.width == IntrinsicSize.Min) {
            maxIntrinsicWidth = measurable.minIntrinsicWidth(Constraints.getMaxHeight-impl(j));
        } else {
            maxIntrinsicWidth = measurable.maxIntrinsicWidth(Constraints.getMaxHeight-impl(j));
        }
        if (maxIntrinsicWidth < 0) {
            maxIntrinsicWidth = 0;
        }
        return Constraints.Companion.fixedWidth-OenEA2s(maxIntrinsicWidth);
    }
