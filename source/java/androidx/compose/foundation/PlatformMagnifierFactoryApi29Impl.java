...
public final class PlatformMagnifierFactoryApi29Impl implements PlatformMagnifierFactory {
    ...
    public PlatformMagnifierImpl mo513createnHHXs2Y(View view, boolean useTextDefault, long size, float cornerRadius, float elevation, boolean clippingEnabled, Density density, float initialZoom) {
        if (useTextDefault) {
            return new PlatformMagnifierImpl(new Magnifier(view));
        }
        long j = density.toSize-XkaWNTQ(size);
        float f = density.toPx-0680j_4(cornerRadius);
        float f2 = density.toPx-0680j_4(elevation);
        Magnifier.Builder builder = new Magnifier.Builder(view);
        if (j != InlineClassHelperKt.UnspecifiedPackedFloats) {
            builder.setSize(MathKt.roundToInt(Float.intBitsToFloat((int) (j >> 32))), MathKt.roundToInt(Float.intBitsToFloat((int) (j & 4294967295L))));
        }
        if (!Float.isNaN(f)) {
    ...
    public static final class PlatformMagnifierImpl extends PlatformMagnifierFactoryApi28Impl.PlatformMagnifierImpl {
        ...
        public void mo512updateWko1d7g(long sourceCenter, long magnifierCenter, float zoom) {
            if (!Float.isNaN(zoom)) {
