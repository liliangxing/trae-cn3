...
public final class Oklab extends ColorSpace {
    ...
    /* renamed from: M1 */
    private static final float[] f78M1;

    /* renamed from: M2 */
    private static final float[] f79M2;
    ...
    public long toXy$ui_graphics_release(float v0, float v1, float v2) {
        ...
        return (Float.floatToRawIntBits(((fArr2[0] * f5) + (fArr2[3] * f6)) + (fArr2[6] * f7)) << 32) | (Float.floatToRawIntBits((fArr2[1] * f5) + (fArr2[4] * f6) + (fArr2[7] * f7)) & 4294967295L);
    }
    ...
    public long mo5276xyzaToColorJlNiLsg$ui_graphics_release(float x, float y, float z, float a, ColorSpace colorSpace) {
        ...
        float fastCbrt = MathHelpersKt.fastCbrt(f);
        float fastCbrt2 = MathHelpersKt.fastCbrt(f2);
        float fastCbrt3 = MathHelpersKt.fastCbrt(f3);
        float[] fArr2 = f79M2;
        return ColorKt.Color((fArr2[0] * fastCbrt) + (fArr2[3] * fastCbrt2) + (fArr2[6] * fastCbrt3), (fArr2[1] * fastCbrt) + (fArr2[4] * fastCbrt2) + (fArr2[7] * fastCbrt3), (fArr2[2] * fastCbrt) + (fArr2[5] * fastCbrt2) + (fArr2[8] * fastCbrt3), a, colorSpace);
    }

    @Override // androidx.compose.p002ui.graphics.colorspace.ColorSpace
    public float[] fromXyz(float[] v) {
        ColorSpaceKt.mul3x3Float3(f78M1, v);
        v[0] = MathHelpersKt.fastCbrt(v[0]);
        v[1] = MathHelpersKt.fastCbrt(v[1]);
        v[2] = MathHelpersKt.fastCbrt(v[2]);
        ColorSpaceKt.mul3x3Float3(f79M2, v);
        return v;
    }
