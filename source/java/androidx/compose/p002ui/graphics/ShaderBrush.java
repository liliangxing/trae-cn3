...
public abstract class ShaderBrush extends Brush {
    ...
    public abstract Shader mo4808createShaderuvyYCjk(long size);

    public ShaderBrush() {
        super(null);
        this.createdSize = Size.INSTANCE.m4670getUnspecifiedNHjbRc();
    }
    ...
    public final void m5166setTransformQ8lPUPs(float[] fArr) {
        this.transform = fArr;
        TransformShader transformShader = this.internalTransformShader;
        if (transformShader != null) {
            transformShader.m5250transformQ8lPUPs(fArr);
        }
    }
    ...
    public final void mo4785applyToPq9zytI(long size, Paint p, float alpha) {
        TransformShader transformShader = this.internalTransformShader;
        if (transformShader == null || !Size.m4658equalsimpl0(this.createdSize, size)) {
            if (Size.m4664isEmptyimpl(size)) {
                ...
                transformShader.setShader(mo4808createShaderuvyYCjk(size));
                this.internalTransformShader = transformShader;
                this.createdSize = size;
            }
        }
        if (!Color.m4840equalsimpl0(p.mo4708getColor0d7_KjU(), Color.INSTANCE.m4865getBlack0d7_KjU())) {
            p.mo4714setColor8_81llA(Color.INSTANCE.m4865getBlack0d7_KjU());
        }
