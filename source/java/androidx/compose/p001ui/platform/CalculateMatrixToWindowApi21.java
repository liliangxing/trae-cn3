...
public final class CalculateMatrixToWindowApi21 implements CalculateMatrixToWindow {
    ...
    @Override // androidx.compose.p001ui.platform.CalculateMatrixToWindow
    /* renamed from: calculateMatrixToWindow-EL8BTi8 */
    public void mo1711calculateMatrixToWindowEL8BTi8(View view, float[] matrix) {
        Matrix.reset-impl(matrix);
        m1714transformMatrixToWindowEL8BTi8(view, matrix);
    }

    /* renamed from: transformMatrixToWindow-EL8BTi8, reason: not valid java name */
    private final void m1714transformMatrixToWindowEL8BTi8(View view, float[] matrix) {
        Object parent = view.getParent();
        if (parent instanceof View) {
            m1714transformMatrixToWindowEL8BTi8((View) parent, matrix);
            m1713preTranslate3XD1CNM(matrix, -view.getScrollX(), -view.getScrollY());
            m1713preTranslate3XD1CNM(matrix, view.getLeft(), view.getTop());
        } else {
            view.getLocationInWindow(this.tmpLocation);
            m1713preTranslate3XD1CNM(matrix, -view.getScrollX(), -view.getScrollY());
            m1713preTranslate3XD1CNM(matrix, r0[0], r0[1]);
        }
        android.graphics.Matrix matrix2 = view.getMatrix();
        if (matrix2.isIdentity()) {
            return;
        }
        m1712preConcattUYjHk(matrix, matrix2);
    }

    /* renamed from: preConcat-tU-YjHk, reason: not valid java name */
    private final void m1712preConcattUYjHk(float[] fArr, android.graphics.Matrix matrix) {
        AndroidMatrixConversions_androidKt.setFrom-tU-YjHk(this.tmpMatrix, matrix);
        AndroidComposeView_androidKt.m1701preTransformJiSxe2E(fArr, this.tmpMatrix);
    }

    /* renamed from: preTranslate-3XD1CNM, reason: not valid java name */
    private final void m1713preTranslate3XD1CNM(float[] fArr, float f, float f2) {
        AndroidComposeView_androidKt.m1702preTranslatecG2Xzmc(fArr, f, f2, this.tmpMatrix);
    }
}
