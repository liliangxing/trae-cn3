...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m6518getMinimpl(j);
        this.selectionEnd = TextRange.m6517getMaximpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        int m6518getMinimpl = TextRange.m6518getMinimpl(j);
        int m6517getMaximpl = TextRange.m6517getMaximpl(j);
        if (m6518getMinimpl < 0 || m6518getMinimpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("start (" + m6518getMinimpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (m6517getMaximpl < 0 || m6517getMaximpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("end (" + m6517getMaximpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (m6518getMinimpl > m6517getMaximpl) {
            throw new IllegalArgumentException("Do not set reversed range: " + m6518getMinimpl + " > " + m6517getMaximpl);
        }
    }
    ...
    public final TextRange m6684getCompositionMzsxiRA$ui_text() {
        if (hasComposition$ui_text()) {
            return TextRange.m6508boximpl(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd));
        }
        return null;
    }

    /* renamed from: getSelection-d9O1mEE$ui_text, reason: not valid java name */
    public final long m6685getSelectiond9O1mEE$ui_text() {
    ...
    public final void delete$ui_text(int start, int end) {
        ...
        long m6686updateRangeAfterDeletepWDy79M = EditingBufferKt.m6686updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), TextRange);
        setSelectionStart(TextRange.m6518getMinimpl(m6686updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m6517getMaximpl(m6686updateRangeAfterDeletepWDy79M));
        if (hasComposition$ui_text()) {
            long m6686updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m6686updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), TextRange);
            if (TextRange.m6514getCollapsedimpl(m6686updateRangeAfterDeletepWDy79M2)) {
                commitComposition$ui_text();
            } else {
                this.compositionStart = TextRange.m6518getMinimpl(m6686updateRangeAfterDeletepWDy79M2);
                this.compositionEnd = TextRange.m6517getMaximpl(m6686updateRangeAfterDeletepWDy79M2);
            }
        }
    }
