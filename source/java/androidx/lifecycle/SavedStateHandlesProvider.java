...
public final class SavedStateHandlesProvider implements SavedStateRegistry.SavedStateProvider {
    ...
    public final void performRestore() {
        ...
        if (consumeRestoredStateForKey != null) {
            SavedStateWriter.m3114putAllimpl(m3110constructorimpl, consumeRestoredStateForKey);
        }
    ...
    public final Bundle consumeRestoredStateForKey(String key) {
        ...
        performRestore();
        Bundle bundle = this.restoredState;
        if (bundle == null || (!SavedStateReader.m3024containsimpl(SavedStateReader.m3023constructorimpl(bundle), key))) {
            return null;
        }
        Bundle m3085getSavedStateOrNullimpl = SavedStateReader.m3085getSavedStateOrNullimpl(SavedStateReader.m3023constructorimpl(bundle), key);
        if (m3085getSavedStateOrNullimpl == null) {
            Map emptyMap = MapsKt.emptyMap();
            if (emptyMap.isEmpty()) {
                pairArr = new Pair[0];
            } else {
            ...
            m3085getSavedStateOrNullimpl = BundleKt.bundleOf((Pair[]) Arrays.copyOf(pairArr, pairArr.length));
            SavedStateWriter.m3110constructorimpl(m3085getSavedStateOrNullimpl);
        }
        ...
        return m3085getSavedStateOrNullimpl;
    }
    ...
    public Bundle saveState() {
        ...
        for (Map.Entry<String, SavedStateHandle> entry2 : getViewModel().getHandles().entrySet()) {
            ...
            if (!SavedStateReader.m3101isEmptyimpl(SavedStateReader.m3023constructorimpl(saveState))) {
                SavedStateWriter.m3137putSavedStateimpl(m3110constructorimpl, key, saveState);
            }
        }
        this.restored = false;
        return bundleOf;
