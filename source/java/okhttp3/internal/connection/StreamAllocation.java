...
public final class StreamAllocation {
    ...
    private int canceledReason = TTOkHttpConstants.ERR_TTNET_UNKNOWN;
    ...
    public HttpCodec newStream(OkHttpClient okHttpClient, Interceptor.Chain chain, boolean z) {
        ...
        Call call = this.call;
        if (call != null && call.request() != null && this.call.request().url() != null && this.call.request().url().host() != null) {
            pingIntervalMillis = okHttpClient.pingIntervalMillisWithHostPort(this.call.request().url().host(), this.call.request().url().port());
        }
        try {
            HttpCodec newCodec = findHealthyConnection(connectTimeoutMillis, readTimeoutMillis, writeTimeoutMillis, pingIntervalMillis, okHttpClient.retryOnConnectionFailure(), z).newCodec(okHttpClient, chain, this);
    ...
    private RealConnection findConnection(int i, int i2, int i3, int i4, boolean z) throws IOException {
        ...
        if (route != null || ((selection = this.routeSelection) != null && selection.hasNext())) {
            ...
            if (TTConfigManager.inst().getReconnctTimeout() <= 0 || !TTConfigManager.inst().isIpReconnQuickTestEnable()) {
                this.routeSelection.setIsQuickTestPhase(false);
            }
            z3 = true;
        }
        synchronized (this.connectionPool) {
        ...
        int reconnctTimeout = (selection2 == null || !selection2.isQuickTestPhase()) ? i : TTConfigManager.inst().getReconnctTimeout();
        if (this.routeSelection == null && this.route != null && TTConfigManager.inst().isIpReconnQuickTestEnable() && TTConfigManager.inst().getReconnctTimeout() > 0) {
            reconnctTimeout = TTConfigManager.inst().getReconnctTimeout();
        }
        realConnection2.connect(reconnctTimeout, i2, i3, i4, z, this.call, this.eventListener);
    ...
    public void resetRouteSelector(int i) {
        this.routeSelector.resetNextProxy(i);
