        ...
        if (j3 > 0) {
            if (((-j3) & j3) == j3) {
                ...
                if (i != 0) {
                    ...
                    j2 = (nextBits(RandomKt.fastLog2(i2)) << 32) + (nextInt() & BodyPartID.bodyIdMax);
                    return from + j2;
                }
                j2 = nextInt & BodyPartID.bodyIdMax;
                return from + j2;
            }
            do {
    ...
    */
    public double nextDouble(double from, double until) {
        ...
        if (Double.isInfinite(d)) {
            if (Math.abs(from) <= Double.MAX_VALUE) {
                if (Math.abs(until) <= Double.MAX_VALUE) {
                    double d2 = 2;
