...
final class LimitedFileDescriptorHardwareBitmapService implements HardwareBitmapService {
    ...
    public boolean allowHardwareMainThread(Size size) {
        ...
        if ((width instanceof Dimension.Pixels ? ((Dimension.Pixels) width).m3269unboximpl() : Integer.MAX_VALUE) > 100) {
            Dimension height = size.getHeight();
            if ((height instanceof Dimension.Pixels ? ((Dimension.Pixels) height).m3269unboximpl() : Integer.MAX_VALUE) > 100) {
                return true;
            }
        }
        return false;
