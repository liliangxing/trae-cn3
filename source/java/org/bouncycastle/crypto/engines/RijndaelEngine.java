...
public class RijndaelEngine implements BlockCipher {
    ...
    private byte mul0xe(int i) {
        if (i >= 0) {
            return aLogtable[i + MediaPlayer.MEDIA_PLAYER_OPTION_OUTPUT_LOG];
        }
        return (byte) 0;
    }
    ...
    private void unpackBlock(byte[] bArr, int i) {
        ...
        for (int i3 = 8; i3 != this.BC; i3 += 8) {
            this.A0 |= (bArr[i2] & UByte.MAX_VALUE) << i3;
            this.A1 |= (bArr[r3] & UByte.MAX_VALUE) << i3;
            this.A2 |= (bArr[r8] & UByte.MAX_VALUE) << i3;
            i2 = i2 + 1 + 1 + 1 + 1;
            this.A3 |= (bArr[r3] & UByte.MAX_VALUE) << i3;
        }
    }
