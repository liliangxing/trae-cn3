...
public class KDFFeedbackBytesGenerator implements MacDerivationFunction {
    ...
    public KDFFeedbackBytesGenerator(Mac mac) {
        ...
        this.f930h = macSize;
        this.f932k = new byte[macSize];
    }

    private void generateNext() {
        if (this.generatedBytes == 0) {
            Mac mac = this.prf;
            byte[] bArr = this.f931iv;
            mac.update(bArr, 0, bArr.length);
        } else {
            Mac mac2 = this.prf;
            byte[] bArr2 = this.f932k;
            mac2.update(bArr2, 0, bArr2.length);
        }
        if (this.useCounter) {
            int i = (this.generatedBytes / this.f930h) + 1;
            byte[] bArr3 = this.ios;
    ...
    public int generateBytes(byte[] bArr, int i, int i2) throws DataLengthException, IllegalArgumentException {
        ...
        System.arraycopy(this.f932k, i7, bArr, i, min);
        ...
        while (true) {
            ...
            generateNext();
            min = Math.min(this.f930h, i8);
            System.arraycopy(this.f932k, 0, bArr, i, min);
            this.generatedBytes += min;
            i8 -= min;
        }
    }
    ...
    public void init(DerivationParameters derivationParameters) {
        ...
        if (kDFFeedbackParameters.useCounter()) {
            BigInteger multiply = TWO.pow(r).multiply(BigInteger.valueOf(this.f930h));
            if (multiply.compareTo(INTEGER_MAX) != 1) {
                i = multiply.intValue();
            }
        }
        this.maxSizeExcl = i;
        this.f931iv = kDFFeedbackParameters.getIV();
