...
public class LMSPublicKeyParameters extends LMSKeyParameters implements LMSContextBasedVerifier {
    ...
    public LMSPublicKeyParameters(LMSigParameters lMSigParameters, LMOtsParameters lMOtsParameters, byte[] bArr, byte[] bArr2) {
        ...
        this.f1371I = Arrays.clone(bArr2);
        this.f1372T1 = Arrays.clone(bArr);
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (this.parameterSet.equals(lMSPublicKeyParameters.parameterSet) && this.lmOtsType.equals(lMSPublicKeyParameters.lmOtsType) && Arrays.areEqual(this.f1371I, lMSPublicKeyParameters.f1371I)) {
            return Arrays.areEqual(this.f1372T1, lMSPublicKeyParameters.f1372T1);
        }
        return false;
    }
    ...
    public LMSContext generateOtsContext(LMSSignature lMSSignature) {
        ...
        if (lMSSignature.getOtsSignature().getType().getType() == type) {
            return new LMOtsPublicKey(LMOtsParameters.getParametersForType(type), this.f1371I, lMSSignature.getQ(), null).createOtsContext(lMSSignature);
        }
    ...
    public byte[] getI() {
        return Arrays.clone(this.f1371I);
    }
    ...
    public byte[] getT1() {
        return Arrays.clone(this.f1372T1);
    }

    public int hashCode() {
        return (((((this.parameterSet.hashCode() * 31) + this.lmOtsType.hashCode()) * 31) + Arrays.hashCode(this.f1371I)) * 31) + Arrays.hashCode(this.f1372T1);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public boolean matchesT1(byte[] bArr) {
        return Arrays.constantTimeAreEqual(this.f1372T1, bArr);
    }
    ...
    public byte[] toByteArray() {
        return Composer.compose().u32str(this.parameterSet.getType()).u32str(this.lmOtsType.getType()).bytes(this.f1371I).bytes(this.f1372T1).build();
    }
