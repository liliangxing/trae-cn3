...
public class McElieceCipher implements MessageEncryptor {
    ...
    private GF2Vector computeMessageRepresentative(byte[] bArr) {
        byte[] bArr2 = new byte[this.maxPlainTextSize + ((this.f1389k & 7) != 0 ? 1 : 0)];
        System.arraycopy(bArr, 0, bArr2, 0, bArr.length);
        bArr2[bArr.length] = 1;
        return GF2Vector.OS2VP(this.f1389k, bArr2);
    }

    private void initCipherDecrypt(McEliecePrivateKeyParameters mcEliecePrivateKeyParameters) {
        this.f1390n = mcEliecePrivateKeyParameters.getN();
        int k = mcEliecePrivateKeyParameters.getK();
        this.f1389k = k;
        this.maxPlainTextSize = k >> 3;
        this.cipherTextSize = this.f1390n >> 3;
    }

    private void initCipherEncrypt(McEliecePublicKeyParameters mcEliecePublicKeyParameters) {
        this.f1390n = mcEliecePublicKeyParameters.getN();
        this.f1389k = mcEliecePublicKeyParameters.getK();
        this.f1392t = mcEliecePublicKeyParameters.getT();
        this.cipherTextSize = this.f1390n >> 3;
        this.maxPlainTextSize = this.f1389k >> 3;
    }
    ...
    public void init(boolean z, CipherParameters cipherParameters) {
        ...
        if (!z) {
            ...
            if (!(cipherParameters instanceof ParametersWithRandom)) {
                this.f1391sr = CryptoServicesRegistrar.getSecureRandom();
            ...
            this.f1391sr = parametersWithRandom.getRandom();
    ...
    public byte[] messageDecrypt(byte[] bArr) throws InvalidCipherTextException {
        ...
        GF2Vector OS2VP = GF2Vector.OS2VP(this.f1390n, bArr);
        ...
        return computeMessage((GF2Vector) sInv.leftMultiply(gF2Vector2.extractRightVector(this.f1389k)));
    }
    ...
    public byte[] messageEncrypt(byte[] bArr) {
        ...
        return ((GF2Vector) ((McEliecePublicKeyParameters) this.key).getG().leftMultiply(computeMessageRepresentative).add(new GF2Vector(this.f1390n, this.f1392t, this.f1391sr))).getEncoded();
    }
}
