...
public final class RoundCornersHorizontalVectorShape implements QrVectorShapeModifier {
    ...
    public Path createPath(float size, Neighbors neighbors) {
        ...
        if (neighbors.getLeft()) {
            path.addRect(0.0f, coerceIn, size / 2, size - coerceIn, Path.Direction.CW);
        } else {
            float f = size / 2;
            path.addCircle(f, f, (size / 2.0f) - coerceIn, Path.Direction.CW);
        }
        if (neighbors.getRight()) {
            path.addRect(size / 2, coerceIn, size, size - coerceIn, Path.Direction.CW);
        } else {
            float f2 = size / 2;
