...
public class ZipOutputStream extends FilterOutputStream {
    ...
    private void addUnicodeExtraFields(ZipEntry zipEntry, boolean z, ByteBuffer byteBuffer) throws IOException {
        ...
        if (this.createUnicodeExtraFields == UnicodeExtraFieldPolicy.ALWAYS || !this.zipEncoding.canEncode(comment)) {
    ...
    private byte[] createCentralFileHeader(ZipEntry zipEntry) throws IOException {
        ...
        boolean z = hasZip64Extra(zipEntry) || zipEntry.getCompressedSize() >= 4294967295L || zipEntry.getSize() >= 4294967295L || longValue >= 4294967295L || this.zip64Mode == Zip64Mode.Always;
    ...
    private byte[] createCentralFileHeader(ZipEntry zipEntry, ByteBuffer byteBuffer, long j, boolean z) throws IOException {
        ...
        if (zipEntry.getCompressedSize() >= 4294967295L || zipEntry.getSize() >= 4294967295L || this.zip64Mode == Zip64Mode.Always) {
    ...
    protected void writeCentralDirectoryEnd() throws IOException {
        ...
        byte[] bytes = ZipShort.getBytes(Math.min(size, TypedValue.DENSITY_NONE));
        writeCounted(bytes);
        writeCounted(bytes);
