...
public class UrlRecord {
    ...
    private String getKey() {
        if (this.key == null) {
            StringBuilder append = new StringBuilder().append(this.url).append(RomUtils.SEPARATOR);
            String str = this.f82ip;
            if (str == null) {
                str = "";
            }
            this.key = append.append(str).append(RomUtils.SEPARATOR).append(this.isMainUrl).toString();
        }
        return this.key;
    }
    ...
    public String toString() {
        return "UrlRecord{url='" + this.url + "', ip='" + this.f82ip + "', ipFamily='" + this.ipFamily + "', isMainUrl=" + this.isMainUrl + ", failedTimes=" + this.failedTimes + ", isCurrentFailed=" + this.isCurrentFailed + '}';
    }
}
