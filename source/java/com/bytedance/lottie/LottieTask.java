...
public class LottieTask<T> {
    ...
    public void notifyFailureListeners(Throwable th) {
        ...
        if (arrayList.isEmpty()) {
            Log.w(C0225L.TAG, "Lottie encountered an error but no failure listener was added.", th);
            return;
        }
    ...
    private synchronized void startTaskObserverIfNeeded() {
        if (!taskObserverAlive() && this.result == null) {
            Thread thread = new Thread("LottieTaskObserver") { // from class: com.bytedance.lottie.LottieTask.2
            ...
            thread.start();
            C0225L.debug("Starting TaskObserver thread");
        }
    }
    ...
    public synchronized void stopTaskObserverIfNeeded() {
        if (taskObserverAlive()) {
            if (this.successListeners.isEmpty() || this.result != null) {
                ...
                C0225L.debug("Stopping TaskObserver thread");
            }
        }
    }
