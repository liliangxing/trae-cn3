...
public abstract class BaseStrokeContent implements BaseKeyframeAnimation.AnimationListener, KeyPathElementContent, DrawingContent {
    ...
    /* renamed from: pm */
    private final PathMeasure f38pm = new PathMeasure();
    ...
    public void draw(Canvas canvas, Matrix matrix, int i) {
        C0225L.beginSection("StrokeContent#draw");
        ...
        if (this.paint.getStrokeWidth() <= 0.0f) {
            C0225L.endSection("StrokeContent#draw");
            return;
        }
        ...
        for (int i2 = 0; i2 < this.pathGroups.size(); i2++) {
            ...
            if (pathGroup.trimPath != null) {
                ...
                C0225L.beginSection("StrokeContent#buildPath");
                this.path.reset();
                for (int size = pathGroup.paths.size() - 1; size >= 0; size--) {
                    this.path.addPath(((PathContent) pathGroup.paths.get(size)).getPath(), matrix);
                }
                C0225L.endSection("StrokeContent#buildPath");
                C0225L.beginSection("StrokeContent#drawPath");
                canvas.drawPath(this.path, this.paint);
                C0225L.endSection("StrokeContent#drawPath");
            }
        }
        C0225L.endSection("StrokeContent#draw");
    }
    ...
    private void applyTrimPath(Canvas canvas, PathGroup pathGroup, Matrix matrix) {
        C0225L.beginSection("StrokeContent#applyTrimPath");
        if (pathGroup.trimPath == null) {
            C0225L.endSection("StrokeContent#applyTrimPath");
            return;
        }
        this.path.reset();
        for (int size = pathGroup.paths.size() - 1; size >= 0; size--) {
        ...
        this.f38pm.setPath(this.path, false);
        float length = this.f38pm.getLength();
        while (this.f38pm.nextContour()) {
            length += this.f38pm.getLength();
        }
        ...
        for (int size2 = pathGroup.paths.size() - 1; size2 >= 0; size2--) {
            ...
            this.f38pm.setPath(this.trimPathPath, false);
            float length2 = this.f38pm.getLength();
    ...
    public void getBounds(RectF rectF, Matrix matrix) {
        C0225L.beginSection("StrokeContent#getBounds");
        this.path.reset();
        ...
        C0225L.endSection("StrokeContent#getBounds");
    }

    private void applyDashPatternIfNeeded(Matrix matrix) {
        C0225L.beginSection("StrokeContent#applyDashPattern");
        if (this.dashPatternAnimations.isEmpty()) {
            C0225L.endSection("StrokeContent#applyDashPattern");
            return;
        }
