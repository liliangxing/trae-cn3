...
public class SsoAuthorizeQRCodeLoginJob extends BaseAccountApi<AuthorizeQRCodeLoginResponse> {
    ...
    public SsoAuthorizeQRCodeLoginJob(Context context, ApiRequest apiRequest, AbsApiCall<AuthorizeQRCodeLoginResponse> absApiCall) {
        super(context, apiRequest, absApiCall);
    }

    public static SsoAuthorizeQRCodeLoginJob authorizeLogin(Context context, String str, String str2, String str3, Map<String, String> map, AuthorizeQRCodeLoginCallback authorizeQRCodeLoginCallback) {
        return new SsoAuthorizeQRCodeLoginJob(context, new ApiRequest.Builder().parameter("token", str).parameter("decision", str2).parameter(CsrfTokenManager.SP_KEY_CSRF_TOKEN, str3).parameters(map).url(SpecialNetConstants.getSsoAuthorizeQRCodeToLoginPath()).post(), authorizeQRCodeLoginCallback);
    }

    public static SsoAuthorizeQRCodeLoginJob authorizeLoginWithConfirmedUrl(Context context, String str, String str2, String str3, String str4, Map<String, String> map, AuthorizeQRCodeLoginCallback authorizeQRCodeLoginCallback) {
        if (!TextUtils.isEmpty(str4)) {
            if (!str4.startsWith("http") && !str4.startsWith("https")) {
                str4 = BDAccountNetApi.getUrl(str4);
            }
            return new SsoAuthorizeQRCodeLoginJob(context, new ApiRequest.Builder().parameter("token", str).parameter("decision", str2).parameters(map).url(str4).post(), authorizeQRCodeLoginCallback);
        }
