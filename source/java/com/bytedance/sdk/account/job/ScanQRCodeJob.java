...
public class ScanQRCodeJob extends BaseAccountApi<ScanQRCodeResponse> {
    ...
    public ScanQRCodeJob(Context context, ApiRequest apiRequest, AbsApiCall<ScanQRCodeResponse> absApiCall) {
    ...
    public static ScanQRCodeJob scanQRCode(Context context, String str, Map<String, String> map, ScanQRCodeCallback scanQRCodeCallback) {
        return new ScanQRCodeJob(context, new ApiRequest.Builder().parameter("token", str).parameters(map).url(SpecialNetConstants.getAuthorizeScanQRCodePath()).post(), scanQRCodeCallback);
    }
