...
final class WebSocketReader {
    ...
    int opcode;
    private final long readLimitLength;
    ...
    public void processNextFrame() throws IOException {
        readHeader();
        if (this.frameLength > this.readLimitLength) {
            Logger.m190d(OkChannelImpl.TAG, "frame too large,skip");
            throwFrame(this.frameLength);
        } else if (this.isControlFrame) {
            readControlFrame();
        } else {
            readMessageFrame();
        }
    ...
    private void throwFrame(long j) {
        try {
            this.source.skip(j);
        } catch (Exception e) {
            e.printStackTrace();
