...
public abstract class AbstractAppLogLogger implements IAppLogLogger {
    ...
    @Override // com.bytedance.tobshadow.applog.log.IAppLogLogger
    ...
    public void process(int i, int i2, List<String> list, Throwable th, String str, Object... objArr) {
        ...
        LogInfo build = LogInfo.builder().appId(this.appId).category(i).level(i2).thread(Thread.currentThread().getName()).throwable(th).tags(getTags(list)).message(format(str, objArr)).build();
        ...
        try {
            ...
            if (objArr != null && objArr.length != 0 && str.contains("{}")) {
                ...
                while (i < length) {
                    ...
                    char charAt2 = i < length + (-1) ? str.charAt(i + 1) : CHAR_PLACEHOLDER;
                    if (charAt == '{' && charAt2 == '}') {
                        if (i2 < objArr.length) {
