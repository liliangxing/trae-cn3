...
public class WXAPiSecurityHelper {
    ...
    /* renamed from: sp */
    private final SharedPreferences f206sp;
    ...
    public static class PromiseShareRule {
        ...
        public long getLegalReqInterval() {
            ...
            if (j <= 0 || j >= 86400) {
                j = TimeUtils.SECONDS_PER_HOUR;
            }
            Log.m269d(WXAPiSecurityHelper.TAG, "getLegalReqInterval = " + j);
            return j;
        }
        ...
        public long getLegalUserWaitTime() {
            ...
            Log.m269d(WXAPiSecurityHelper.TAG, "getLegalUserWaitTime = " + j);
            return j;
        }
    }
    ...
    public static class WXSecurityData {
        ...
        public boolean isBasicParamsAllowed() {
            ...
            return ((j > 0L ? 1 : (j == 0L ? 0 : -1)) > 0 && (j > (System.currentTimeMillis() / 1000) ? 1 : (j == (System.currentTimeMillis() / 1000) ? 0 : -1)) < 0) && (C1007b.m278b(this.appid) ^ true) && (C1007b.m278b(this.respDataJson) ^ true);
        }
    }

    public WXAPiSecurityHelper(Context context) {
        this.context = context;
        this.f206sp = context.getSharedPreferences("__wx_opensdk_sp__", 0);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public int checkRuleCanPass(String str, PromiseShareRule promiseShareRule) {
        String str2;
        Log.m269d(TAG, "checkRuleCanPass, start check!");
        if (promiseShareRule.orgJsonData.length() > 2048) {
            Log.m273w(TAG, "response json is too long!");
            return 0;
        }
        ...
        if (i == 2) {
            ...
            if (i != 1 || !promiseShareRule.appidInRule.equals(promiseShareRule.reqAppid)) {
                Log.m269d(TAG, "checkRuleCanPass, unknown");
                return 0;
            }
            if (promiseShareRule.urlRuleList.size() == 0) {
                ...
                Log.m269d(TAG, "checkRuleCanPass, urlCheckResult = " + checkUrlParametersLegal);
                if (!checkUrlParametersLegal) {
                    Log.m269d(TAG, "checkRuleCanPass, no pass");
                    return 2;
                }
    ...
    private boolean checkUrlParametersLegal(String str, PromiseShareRule promiseShareRule) {
        ...
        if (C1007b.m278b(host)) {
            Log.m271i(TAG, "checkUrlParameters, host empty!");
            return false;
        }
            ...
            if (host.equals(urlRule.host)) {
                ...
                for (int i3 = 0; i3 < urlRule.mustQueryKey.size(); i3++) {
                    try {
                        ...
                        Log.m269d(TAG, "checkRuleCanPass, key = " + str2);
                        if (!C1007b.m278b(parse.getQueryParameter(str2))) {
                            i2++;
                        }
                    } catch (Exception e) {
                        Log.m270e(TAG, "checkRuleCanPass, parse fail, e = %s" + e.getMessage());
                    }
                }
                boolean z = i2 == urlRule.mustQueryKey.size();
                Log.m269d(TAG, "checkRuleCanPass, rule.host = " + urlRule.host + ", queryOk = " + z);
                return z;
            }
        }
        return false;
    ...
    private void doRequest(final PassContext passContext, final IHttpCheckCallback iHttpCheckCallback) {
        C1007b.f209b.submit(new Runnable() { // from class: com.tencent.mm.opensdk.openapi.WXAPiSecurityHelper.6
    ...
    private void doRequestSync(final PassContext passContext, final ISecuritySyncCheck iSecuritySyncCheck) {
        Log.m271i(TAG, "requestId = " + passContext.reqSessionId);
        ...
        timer.schedule(new TimerTask() { // from class: com.tencent.mm.opensdk.openapi.WXAPiSecurityHelper.4
            ...
            public void run() {
                Log.m271i(WXAPiSecurityHelper.TAG, "sync request overtime, requestId = " + passContext.reqSessionId);
    ...
    public void extraSecurityCheckDoCallback(final ISecurityCheck iSecurityCheck, final boolean z) {
        if (iSecurityCheck != null) {
            new Handler(Looper.getMainLooper()).post(new Runnable() { // from class: com.tencent.mm.opensdk.openapi.WXAPiSecurityHelper.2
                @Override // java.lang.Runnable
                public void run() {
                    Log.m269d(WXAPiSecurityHelper.TAG, "has got result, callback on Main Thread.");
    ...
    private void storeCheckResp(String str, PromiseShareRule promiseShareRule) {
        ...
        Log.m269d(TAG, "cookStoreJson = " + cookStoreJson);
        if (C1007b.m278b(cookStoreJson)) {
            Log.m273w(TAG, "cookStoreJson get null!");
            return;
        }
        try {
            ...
            Log.m270e(TAG, "storeCheckResp fail, ex = " + e.getMessage());
        }
    }
    ...
    public void storeIfNecessary(PromiseShareRule promiseShareRule) {
        String str;
        Log.m269d(TAG, "storeIfNecessary");
        if (C1007b.m278b(promiseShareRule.reqAppid) || promiseShareRule.reqAppid.length() > 100) {
            str = "store fail! reqAppid illegal!";
        } else {
            if (!C1007b.m278b(promiseShareRule.orgJsonData) && promiseShareRule.orgJsonData.length() <= 2048) {
                if (promiseShareRule.state == 1 && promiseShareRule.reqAppid.equals(promiseShareRule.appidInRule)) {
                    Log.m269d(TAG, "storeIfNecessary, in rule");
                    storeCheckResp(promiseShareRule.reqAppid, promiseShareRule);
                }
                if (promiseShareRule.state == 2) {
                    Log.m269d(TAG, "storeIfNecessary, not rule");
                    storeCheckResp(promiseShareRule.reqAppid, promiseShareRule);
                    return;
                }
                return;
    ...
    protected void commitSp(String str, String str2) {
        SharedPreferences.Editor edit = this.f206sp.edit();
    ...
    protected PromiseShareRule convert2ShareRule(String str, String str2) {
        ...
        if (C1007b.m278b(str2)) {
            Log.m269d(TAG, "convert2ShareRule: jsonRespData is empty");
        ...
        try {
                ...
                for (int i = 0; i < optJSONArray.length(); i++) {
                    ...
                    if (optJSONArray2 != null) {
                        for (int i2 = 0; i2 < optJSONArray2.length(); i2++) {
                            if (!C1007b.m278b(optJSONArray2.optString(i2, ""))) {
            ...
            Log.m270e(TAG, "coverJson2ShareRule fail, ex = " + e.getMessage());
        }
        return promiseShareRule;
    }
    ...
    protected String cookStoreJson(String str, PromiseShareRule promiseShareRule) {
        ...
        if (promiseShareRule != null && !C1007b.m278b(str)) {
            try {
                if (promiseShareRule.state == 2) {
                    JSONObject jSONObject = new JSONObject();
    ...
    protected String covertShareRule2Json(PromiseShareRule promiseShareRule) {
        ...
        try {
            ...
            Log.m270e(TAG, "covertShareRule2Json fail, ex = " + e.getMessage());
            return "";
        }
    }

    public int doExtraSecurityCheck(String str, String str2, String str3, final ISecurityCheck iSecurityCheck) {
        Log.m271i(TAG, "doExtraSecurityCheck: start!");
        if (C1007b.m278b(str) || C1007b.m278b(str3)) {
            Log.m270e(TAG, "doExtraSecurityCheck: appid or inputurl is empty!");
            return 0;
        }
        long currentTimeMillis = System.currentTimeMillis();
        PassContext passContext = new PassContext();
        passContext.reqSessionId = String.format("%s", currentTimeMillis + "" + new Random().nextInt(AVMDLDataLoader.KeyIsEnableEventInfo));
        ...
        if (this.context.getPackageManager().checkPermission("android.permission.INTERNET", this.context.getPackageName()) != 0) {
            Log.m273w(TAG, "doExtraSecurityCheck: No Internet permission!");
            return 0;
        }
        if (!storedData.isBasicParamsAllowed()) {
            Log.m269d(TAG, "doExtraSecurityCheck: local data illegal!");
        ...
        Log.m269d(TAG, "doExtraSecurityCheck: needDoNextReq: " + z + ", last req time stamp:" + storedData.lastStoreTimeStampSecond);
        if (!z) {
            int checkRuleCanPass = checkRuleCanPass(passContext.inputUrl, passContext.localRule);
            Log.m269d(TAG, "doExtraSecurityCheck: no needDoNextReq, read local rule: " + checkRuleCanPass);
            return checkRuleCanPass == 2 ? 1 : 0;
        }
        Log.m269d(TAG, "doExtraSecurityCheck: needDoNextReq, state: " + passContext.localRule.state);
    ...
    public String extractMayNeedDoSecurityCheckUrl(String str, BaseReq baseReq) {
        ...
        if (C1007b.m278b(wXWebpageObject.webpageUrl)) {
            Log.m271i(TAG, "webpageUrl empty, don't need check.");
            return "";
        }
        Log.m271i(TAG, "need check.");
    ...
    protected WXSecurityData getStoredData(String str) {
        ...
        Log.m269d(TAG, "getStoredData, appid = " + str);
        try {
            if (this.f206sp.contains(getStoreKey(str))) {
                JSONObject jSONObject = new JSONObject(this.f206sp.getString(getStoreKey(str), ""));
            ...
            Log.m270e(TAG, "getStoredData fail, ex = " + e.getMessage());
        }
    ...
    protected StringBuffer postHttpRequest(String str, String str2, String str3) {
        ...
        try {
            ...
            Log.m269d(TAG, "request http, url = " + format + ", appid = " + encode + ", version = " + str2 + ", buffer = " + encode2);
            HttpURLConnection httpURLConnection = (HttpURLConnection) new URL(format).openConnection();
            httpURLConnection.setConnectTimeout(AVMDLMultiNetwork.mLoopIpRefreshInterval);
            httpURLConnection.setReadTimeout(AVMDLMultiNetwork.mLoopIpRefreshInterval);
            httpURLConnection.setRequestProperty("Content-Type", "application/json");
            httpURLConnection.connect();
            int responseCode = httpURLConnection.getResponseCode();
            Log.m271i(TAG, "http respCode = " + responseCode);
            if (responseCode == 200) {
                ...
                Log.m269d(TAG, "http response = " + ((Object) stringBuffer));
                bufferedReader.close();
            }
            httpURLConnection.disconnect();
        } catch (Exception e) {
            Log.m270e(TAG, "http request fail, ex = " + e.getMessage());
        }
        return stringBuffer;
    }
}
