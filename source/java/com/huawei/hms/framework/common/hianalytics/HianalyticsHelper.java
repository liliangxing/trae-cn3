...
public class HianalyticsHelper {
    ...
    private HianalyticsHelper() {
        try {
            ...
            Logger.m1185i(TAG, "Hianalytics sdk not found");
        ...
        Logger.m1188v(TAG, "this time the ha %s, mini %s", Boolean.valueOf(this.hasHianalytics), Boolean.valueOf(this.hasHMSBI));
    }

    private void tryHMSBIInit(Context context) {
        if (context == null) {
            Logger.m1185i(TAG, "the appContext hasn't init");
            return;
        }
        try {
            ...
            Logger.m1189w(TAG, "maybe you need add base sdk!");
        } catch (Throwable unused2) {
            Logger.m1189w(TAG, "the hms base has other error!");
        }
    }
    ...
    public void setRate(int i) {
        ...
        this.bReportable = z;
        Logger.m1185i(TAG, "bReportable = " + this.bReportable + ", inuser = " + this.bInstallWelink + ", rate = " + i);
    }
    ...
    private boolean isEnableReport(Context context, boolean z, boolean z2) {
        ...
        try {
            ...
            Logger.m1189w(TAG, "the setting has illegalStateException");
        } catch (Throwable unused2) {
            Logger.m1189w(TAG, "the setting has other error");
        }
        Logger.m1185i(TAG, "user experience involved needs to be opened");
        return false;
    }
    ...
    public void onEvent(LinkedHashMap<String, String> linkedHashMap, String str, int i) {
        ...
        if (CommonActivityUtil.getInstance().isRegistered()) {
            linkedHashMap.put("foreground", "" + (CommonActivityUtil.getInstance().isForeground() ? 1 : 0));
        }
        Logger.m1188v(TAG, "data = %s", linkedHashMap);
        ...
        if (this.hasHMSBI) {
            ...
            Logger.m1188v(TAG, "the base sdk isn't exsit, and reportType is %s", Integer.valueOf(i));
            return;
        }
        if (this.hasHianalytics) {
            ...
            if (hiAnalyticsInstance != null) {
                ...
                Logger.m1182e(TAG, "the ha has error,has init but is null!");
            }
        }
    }
    ...
    private void onNewEvent(Context context, String str, Map map, int i) {
        ...
        Logger.m1188v(TAG, "data = %s", map);
        try {
            HiAnalyticsUtils.getInstance().onNewEvent(context, str, map, i);
        } catch (NoSuchMethodError unused) {
            Logger.m1189w(TAG, "may be you need upgrade stats sdk");
        } catch (Throwable unused2) {
            Logger.m1185i(TAG, "the stats has other error,pls check it");
        }
    }
