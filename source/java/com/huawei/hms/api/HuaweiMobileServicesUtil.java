...
public abstract class HuaweiMobileServicesUtil {
    ...
    public static int isHuaweiMobileServicesAvailable(Context context, int i) {
        ...
        if (HMSPackageManager.getInstance(context).isUseOldCertificate()) {
            HMSLog.m2118e(TAG, "The CP uses the old certificate to terminate the connection.");
            return 13;
        }
        ...
        if (TextUtils.isEmpty(hMSPackageNameForMultiService)) {
            HMSLog.m2121w(TAG, "hmsPackageName is empty, Service is invalid.");
            return 1;
        }
        HMSLog.m2120i(TAG, "hmsPackageName is " + hMSPackageNameForMultiService);
        ...
        if (PackageManagerHelper.PackageStates.NOT_INSTALLED.equals(hMSPackageStatesForMultiService)) {
            HMSLog.m2121w(TAG, "HMS is not installed");
            return 1;
        }
        ...
        HMSLog.m2120i(TAG, "connect versionCode:" + HMSPackageManager.getInstance(context).getHmsMultiServiceVersion());
        if (!HMSPackageManager.getInstance(context).isApkUpdateNecessary(i)) {
            return 0;
        }
        HMSLog.m2120i(TAG, "The current version does not meet the minimum version requirements");
        return 2;
    }
