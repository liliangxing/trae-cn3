...
public class ForwardingRequestListener extends BaseRequestListener {
    ...
    public void onRequestIntermediateResult(ImageRequest imageRequest, String str, boolean z) {
        int size = this.mRequestListeners.size();
        for (int i = 0; i < size; i++) {
            RequestListener requestListener = this.mRequestListeners.get(i);
            try {
                ...
                onException("InternalListener exception in onRequestIntermediateResult", e);
            }
        }
    }
    ...
    private void onException(String str, Throwable th) {
        FLog.m443e(TAG, str, th);
    }

    public byte[] decrypt(InputStream inputStream) {
        int size = this.mRequestListeners.size();
        for (int i = 0; i < size; i++) {
            if (this.mRequestListeners.get(i) instanceof AbsDiskWriteReadListener) {
                return ((AbsDiskWriteReadListener) this.mRequestListeners.get(i)).decrypt(inputStream);
            }
        }
        return null;
    }
