...
public abstract class LocalFetchProducer implements Producer<EncodedImage> {
    ...
    public EncodedImage getByteBufferBackedEncodedImage(InputStream inputStream, int i) throws IOException {
        CloseableReference m496of;
        CloseableReference closeableReference = null;
        try {
            if (i <= 0) {
                m496of = CloseableReference.m496of(this.mPooledByteBufferFactory.newByteBuffer(inputStream));
            } else {
                m496of = CloseableReference.m496of(this.mPooledByteBufferFactory.newByteBuffer(inputStream, i));
            }
            closeableReference = m496of;
