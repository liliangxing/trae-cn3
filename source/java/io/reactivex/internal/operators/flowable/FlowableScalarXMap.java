...
public final class FlowableScalarXMap {
    ...
    public static <T, R> boolean tryScalarXMapSubscribe(Publisher<T> publisher, Subscriber<? super R> subscriber, Function<? super T, ? extends Publisher<? extends R>> function) {
        ...
        try {
            ...
            try {
                Callable callable = (Publisher) ObjectHelper.requireNonNull(function.apply(call), "The mapper returned a null Publisher");
    ...
    static final class ScalarXMapFlowable<T, R> extends Flowable<R> {
        ...
        public void subscribeActual(Subscriber<? super R> subscriber) {
            try {
                Callable callable = (Publisher) ObjectHelper.requireNonNull(this.mapper.apply(this.value), "The mapper returned a null Publisher");
                if (callable instanceof Callable) {
                    try {
