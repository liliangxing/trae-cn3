...
public final class FlowableRefCount<T> extends Flowable<T> {
    ...
    protected void subscribeActual(Subscriber<? super T> subscriber) {
        ...
        synchronized (this) {
            ...
            if (refConnection.connected || j2 != this.f17n) {
                z = false;
            } else {
                z = true;
    ...
    void terminated(RefConnection refConnection) {
        synchronized (this) {
            ...
            refConnection.subscriberCount = j;
            if (j == 0) {
                Disposable disposable = this.source;
                if (disposable instanceof Disposable) {
                    disposable.dispose();
                } else if (disposable instanceof ResettableConnectable) {
                    ((ResettableConnectable) disposable).resetIf(refConnection.get());
                }
            }
        }
    }
    ...
    void timeout(RefConnection refConnection) {
        synchronized (this) {
            if (refConnection.subscriberCount == 0 && refConnection == this.connection) {
                ...
                Disposable disposable2 = this.source;
        ...
        public void accept(Disposable disposable) throws Exception {
            ...
            synchronized (this.parent) {
                if (this.disconnectedEarly) {
                    this.parent.source.resetIf(disposable);
                }
            }
        }
    }
