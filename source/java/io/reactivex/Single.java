...
public abstract class Single<T> implements SingleSource<T> {
    ...
    public static <T> Single<T> amb(Iterable<? extends SingleSource<? extends T>> iterable) {
        ...
        return RxJavaPlugins.onAssembly(new SingleAmb((SingleSource[]) null, iterable));
    }
    ...
    public static <T> Single<T> ambArray(SingleSource<? extends T>... singleSourceArr) {
        if (singleSourceArr.length == 0) {
            return error((Callable<? extends Throwable>) SingleInternalHelper.emptyThrower());
        }
    ...
    public static <T> Single<T> fromObservable(ObservableSource<? extends T> observableSource) {
        ...
        return RxJavaPlugins.onAssembly(new ObservableSingleSingle(observableSource, (Object) null));
    }
    ...
    public static <T> Flowable<T> merge(Publisher<? extends SingleSource<? extends T>> publisher) {
        ObjectHelper.requireNonNull(publisher, "sources is null");
        return RxJavaPlugins.onAssembly(new FlowableFlatMapPublisher(publisher, SingleInternalHelper.toFlowable(), false, CongestionController.MAX_REQUEST_FREQUENCY_REALTIME, Flowable.bufferSize()));
    }
    ...
    public static <T> Flowable<T> mergeDelayError(Publisher<? extends SingleSource<? extends T>> publisher) {
        ObjectHelper.requireNonNull(publisher, "sources is null");
        return RxJavaPlugins.onAssembly(new FlowableFlatMapPublisher(publisher, SingleInternalHelper.toFlowable(), true, CongestionController.MAX_REQUEST_FREQUENCY_REALTIME, Flowable.bufferSize()));
    }
    ...
    @CheckReturnValue
    /* renamed from: as */
    public final <R> R m319as(SingleConverter<T, ? extends R> singleConverter) {
    ...
    public final Single<T> onErrorReturn(Function<Throwable, ? extends T> function) {
        ...
        return RxJavaPlugins.onAssembly(new SingleOnErrorReturn(this, function, (Object) null));
    }
    ...
    @CheckReturnValue
    /* renamed from: to */
    public final <R> R m320to(Function<? super Single<T>, R> function) {
        try {
    ...
    private static <T> Single<T> toSingle(Flowable<T> flowable) {
        return RxJavaPlugins.onAssembly(new FlowableSingleSingle(flowable, (Object) null));
    }
}
